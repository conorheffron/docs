...
Storage
-   Kdb+ Object Store - Native support for reading and querying cloud object storage.
...
The HDB segment locations can be on GCP cloud object storage by using the Kx Insights native object store functionality. In this pattern, the HDB can reside entirely on cloud storage or spread across persistent disk, filestore or cloud storage as required. There is a relatively high latency when using cloud storage compared to local storage, such as local SSD. The performance of kdb+ when working with cloud storage can be improved by taking advantage of the caching feature of the kdb+ native object store. The results of requests to cloud storage can be cached on a local high-performance disk thus increasing performance. The cache directory is continuously monitored and a size limit is maintained by deleting files according to a LRU (least recently used) algorithm. 
...
The Kx Insights native object store functionality allows users to read HDB data from GCS. Cloud object storage is slow relative to local storage such as local SSD. However, the performance of kdb+ when working with GCS can be improved by caching GCS data. Each query to GCS has a financial cost and caching the resulting data can help to reduce this cost. Caching coupled with enabling secondary threads can significantly increase the performance of queries against a HDB on GCS.
...
Service discovery can be managed from within kdb+ or using built in service discovery for Compute Engine or Google Kubernetes Engine (GKE). GCP keeps track of all your application components, their locations, attributes and health status. Compute engine uses instance metadata and GKE organizes services into namespaces. A service must have an address and can have multiple attributes. You can add a health check to any service. A service is unhealthy if the number of times the health check failed is above a threshold. Set a higher threshold for HDBs if you allow long running queries.

Kdb+ can easily interact with the GCP Instance Metadata Service using Kurl. Kurl can be extended to create/query namespaces, discover or register/deregister instances to facilitate service discovery of your kdb+ processes running in your tick environment. For example a kdb+ gateway can fetch from GCP the addresses of healthy RDBs and HDBs. The cloud console also comes with a simple web interface to visualize the status of your kdb+ processes/instances.
